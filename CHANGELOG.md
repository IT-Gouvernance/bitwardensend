...
ships as `1.0.0-betaN` pre-releases (see [README.md](README.md#versioning) for why the
version string matters beyond being a label).
...
- `serve` (local Vault Management API) and `cli` (direct `bw` invocation) backends.
- Native (pure PHP) Send driver, talking to the Bitwarden API directly with no `bw`
  ...
  cannot be reproduced with PHP's available primitives — see the README).
- Configuration secrets (master password, `BW_SESSION`, native driver's API client
  secret and master password) encrypted with the GLPI key.
