...
(function () {
   ...
   /**
    * Show only the configuration fields that apply to the selected access mode.
    *
    * Rows opt in with data-bws-mode="serve", "cli", or a space separated list. A row
    * without the attribute is always visible.
    *
    * Hidden inputs are still submitted, deliberately: switching modes to look around
    * does not wipe the settings belonging to the other mode.
    */
   window.pluginBitwardensendToggleBackend = function () {
      var select = document.getElementById('cfg_backend');
      if (!select) {
         return;
      }

      var mode = select.value;
      var rows = document.querySelectorAll('[data-bws-mode]');
    ...
    * native) — same mechanism as pluginBitwardensendToggleBackend() above,
    * a separate attribute/select pair since the two choices are
    * independent (backend only matters within the "cli" driver).
    */
    ...
    * the access mode selector, and load a followup template when one is picked.
   ...
   document.addEventListener('change', function (event) {
      ...
      if (event.target.id === 'cfg_backend') {
         window.pluginBitwardensendToggleBackend();
         return;
      }
