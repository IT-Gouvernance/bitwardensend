...
(function () {
    ...
    window.pluginBitwardensendDialog = function (itemtype, items_id) {
        const url = `${CFG_GLPI.root_doc}/plugins/bitwardensend/ajax/form.php`;
        // Both the glpi_ajax_dialog title and the fallback modal's <h5> insert
        // this as raw HTML, not escaped text.
        const title = '<i class="ti ti-shield-lock me-2"></i>Bitwarden Send';
        ...
        const id = 'plugin-bitwardensend-modal';
        ...
        wrapper.innerHTML = `<div class="modal fade" id="${id}" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        ...
        const query = `itemtype=${encodeURIComponent(itemtype)}&items_id=${encodeURIComponent(items_id)}`;
    ...
    window.pluginBitwardensendConfirm = function (button) {
        ...
        const title   = button.dataset.bwsTitle || 'Confirm';
        const message = button.dataset.bwsMessage || '';
        const okLabel = button.dataset.bwsConfirm || 'OK';
        const koLabel = button.dataset.bwsCancel || 'Cancel';
        ...
        const id = 'plugin-bitwardensend-confirm';
    ...
    window.pluginBitwardensendToggleSendDriver = function () {
        const select = document.getElementById('cfg_send_driver');
        if (!select) {
            return;
        }

        const driver = select.value;
        const rows = document.querySelectorAll('[data-bws-driver]');

        Array.prototype.forEach.call(rows, (row) => {
            const applies = row.getAttribute('data-bws-driver').split(/\s+/);
    ...
    window.pluginBitwardensendUpdateRequiredFields = function () {
        const fields = document.querySelectorAll('[data-bws-required]');

        Array.prototype.forEach.call(fields, (field) => {
            const wanted = field.getAttribute('data-bws-required') === '1';
    ...
    window.pluginBitwardensendCopy = function (button, value) {
        const done = function () {
            ...
            window.setTimeout(() => { icon.className = previous; }, 1500);
        };
    ...
    window.pluginBitwardensendGeneratePassword = function () {
        const field = document.getElementById('bws_password');
        if (!field) {
            return;
        }
    ...
    window.pluginBitwardensendGenerateSecret = function () {
        const field = document.getElementById('bws_secret');
        if (!field) {
            return;
        }

        const lengthInput = document.getElementById('bws_secret_gen_length');
        let length = parseInt(lengthInput && lengthInput.value, 10) || 20;
        ...
        let charset = classes.reduce((chars, charClass) => {
            const checkbox = document.getElementById(charClass.id);
    ...
    function pluginBitwardensendSanitizeRichText(html) {
        ...
        while (node) {
            ...
            Array.prototype.slice.call(node.attributes || []).forEach((attr) => {
                ...
                // Browsers strip every ASCII tab/newline/CR from anywhere
                // in a URL, plus any leading C0 control character or
                // space, before parsing it for navigation (WHATWG URL
                // Standard's own basic URL parser) - do the same before
                // testing the scheme. Covers both a hidden tab inside the
                // word (e.g. "jav" + tab + "ascript:") and a leading
    ...
    window.pluginBitwardensendUpdateFollowupPreview = function () {
        const block = document.getElementById('bws_followup_block');
        const preview = document.getElementById('bws_followup_preview');
        const daysSelect = document.getElementById('bws_days');
        const maxAccessInput = document.getElementById('bws_max_access');
        ...
        const maxAccess = parseInt(maxAccessInput.value, 10) || 0;
        const expiration = new Date(Date.now() + days * 86400000).toLocaleString();
        const maxAccessText = maxAccess > 0 ? String(maxAccess) : block.dataset.bwsUnlimited;
        ...
        const urlPlaceholderText = escapeHtml(block.dataset.bwsLinkPlaceholder);
        const urlHtml = `<em class="text-secondary">${urlPlaceholderText}</em>`;

        let html = editor ? editor.getContent() : escapeHtml(textarea.value).replace(/\n/g, '<br>');
        ...
        // textarea fallback).
        const templateSelect = document.getElementById('bws_followup_template');
        if (templateSelect) {
    ...
    window.pluginBitwardensendApplyFollowupTemplate = function (select) {
        const editorContent = pluginBitwardensendGetFollowupEditorContent();

        if (editorContent !== null && select._bwsBaseline !== undefined && editorContent !== select._bwsBaseline) {
            const confirmMessage = select.dataset.bwsConfirmReplace || 'Replace the current text?';
     ...
     */
    let followupPreviewInterval = null;
    ...
    document.addEventListener('submit', (event) => {
        ...
        const button = form.querySelector('button[name="create_send"]');
